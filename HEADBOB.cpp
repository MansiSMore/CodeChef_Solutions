...
Usual "Yes" gesture is recorded as "Y", no as "N" and Indian "Yes" gesture as "I". (Assume no foreigner uses the Indian "Yes" gesture and vice-versa). 
...
Each person is described in two lines. First line of the description contains a single integer N, the number of gestures recorded for this person. 
...
int main() {
	...
	while(T--)
	{
	    ...
	    if(s1.find("I") != std::string::npos && s1.find("Y") == std::string::npos ){
	        cout<<"INDIAN"<<endl;
	    }
	    else if(s1.find("Y") != std::string::npos &&  s1.find("I") == std::string::npos )
